---
name: SRR-CBP Implementation
overview: Implement Soft Rank-Restoring Continual Backpropagation (SRR-CBP) using forward hooks to compute the Asymmetric Soft Orthogonality (ASO) loss, reusing existing GnT classes for the replacement phase.
todos:
  - id: config
    content: Add SRRCBPConfig to configs/configurations.py
    status: pending
  - id: learner
    content: Create src/algos/supervised/srr_cbp.py with SRR_CBP_for_ConvNet and SRR_CBP_for_FC
    status: pending
  - id: hooks
    content: Implement forward hooks for pre-activations in learner init
    status: pending
  - id: aso_loss
    content: Implement ASO loss computation in learner learn method
    status: pending
  - id: factory
    content: Register learners in supervised_factory.py
    status: pending
isProject: false
---
...
- **Initialization (`__init__`)**:
  - Initialize the standard `ConvGnT_for_ConvNet` (or `GnT_for_FC`) for utility tracking and unit replacement.
  ...
  - **Compute ASO Loss**:
    ...
    - Loop over each hidden layer `i`:
      ...
      - If both young and mature units exist:
        ...
        - Calculate the penalty weights for young units: `weights = SO_reg_lambda * (aso_age_decay_rate ** ages[young_idx]) / (2 * m**2)`.
